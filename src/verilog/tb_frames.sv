...
module tb_frames;
    ...
    real    pi, th;
    integer qq;
    integer total_errors, ferr, argmax, miss;
    reg [63:0] p, pmax;
    ...
    task check_frame(input integer kbin);
        begin
            ...
            total_errors = total_errors + ferr;
            $display("FRAMES| tone=%0d recv=%0d miss=%0d peak=%0d(exp %0d/%0d) tlast=%0d stray=%0d -> %s",
                     kbin, recv, miss, argmax, kbin, N-kbin, last_cnt, stray_last,
                     (ferr==0) ? "PASS" : "FAIL");
        end
    endtask

    initial begin
        $dumpfile("fft256_frames.vcd");
        $dumpvars(0, tb_frames);
        ...
        total_errors = 0;
        run_frame(13);   check_frame(13);
        run_frame(65);   check_frame(65);
        run_frame(100);  check_frame(100);

        $display("FRAMES| ================ VERDICT ================");
        $display("FRAMES| 3 frames, total errors = %0d -> %s",
